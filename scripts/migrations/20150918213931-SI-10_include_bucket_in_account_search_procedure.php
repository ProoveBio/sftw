 ...
 * @author
 */
class MigrationClass_20150918213931 extends SchemaChange
{
...
,OTHER_D.tricare
		...
		FROM
			...
			LEFT OUTER JOIN tblAccountContact OTHER_AC ON OTHER_AC.account_id = T.account_id
			LEFT OUTER JOIN tblContact OTHER_C ON OTHER_AC.contact_id = OTHER_C.contact_id
			LEFT OUTER JOIN tblDoctor OTHER_D ON OTHER_D.contact_id = OTHER_C.contact_id
		WHERE
			T.status NOT IN ('Terminated', 'Pending')
			AND
			C.contact_type_id = 1
			AND
			OTHER_C.contact_type_id = 3"
	);
