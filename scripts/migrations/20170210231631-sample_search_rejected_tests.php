 ...
 * @author
 */
class MigrationClass_20170210231631 extends SchemaChange
{
...
this:BEGIN
	...
	SET @query_sql := CONCAT("
		SELECT DISTINCT SQL_CALC_FOUND_ROWS
			...
			,S.is_deleted
            ,E.is_deleted AS encounter_deleted
			,E.created_datetime
			,E.created_by
			,S.modified_datetime
			,S.modified_by
            ...
            ,P.is_deleted AS patient_deleted
		...
		FROM
			...
			INNER JOIN tblEncounter E ON S.encounter_id = E.encounter_id
			INNER JOIN tblUser U ON U.user_id = E.created_by
			LEFT JOIN tblAccount A ON A.account_id = E.account_id
			LEFT JOIN tblContact A_C ON A.contact_id = A_C.contact_id
			LEFT JOIN tblPatient P ON P.patient_id = E.patient_id
     		LEFT JOIN tblContact P_C ON P.contact_id = P_C.contact_id
			LEFT JOIN tblDoctor D ON D.doctor_id = E.doctor_id
			LEFT JOIN tblContact D_C ON D.contact_id = D_C.contact_id
			...
			LEFT OUTER JOIN tblTest FAILED_TESTS ON S.sample_id = FAILED_TESTS.sample_ID AND FAILED_TESTS.test_status_id IN (7,9) AND FAILED_TESTS.is_deleted = 0
	...
	IF (s_sample_status_id IS NOT NULL) THEN
		IF (s_sample_status_id  = -2) THEN
			SET @query_sql := CONCAT(@query_sql,"
				AND
				S.sample_status_id IN (1,2,7)");
        ELSE
			SET @query_sql := CONCAT(@query_sql,"
				AND
  ...
  END IF;
	...
	ELSEIF (s_sample_status_id = -2) THEN
	SET @query_sql := CONCAT(@query_sql,"
		ORDER BY S.sample_status_id = 7 DESC, ", sort_field," ", sort_dir);
	ELSE
	SET @query_sql := CONCAT(@query_sql,"
